...
var _RNFBNativeEventEmitter = _interopRequireDefault(require("../RNFBNativeEventEmitter.js"));
var _SharedEventEmitter = _interopRequireDefault(require("../SharedEventEmitter.js"));
var _nativeModule = require("../nativeModule");
var _index = require("../../common/index.js");
var _nullSerialization = require("../nullSerialization.js");
...
function nativeModuleMethodWrapped(namespace, method, argToPrepend, isTurboModule) {
  return (...args) => {
    ...
    const processedArgs = _index.isIOS && isTurboModule ? args.map(arg => (0, _nullSerialization.encodeNullValues)(arg)) : args;
    ...
    if (possiblePromise && typeof possiblePromise === 'object' && 'then' in possiblePromise) {
      ...
      return possiblePromise.catch(nativeError => Promise.reject(new _NativeFirebaseError.default(nativeError, jsStack, namespace)));
    }
...
function initialiseNativeModule(module) {
  ...
  for (let i = 0; i < nativeModuleNames.length; i++) {
    ...
    const nativeModule = (0, _nativeModule.getReactNativeModule)(moduleName);
...
function subscribeToNativeModuleEvent(eventName) {
  if (!NATIVE_MODULE_EVENT_SUBSCRIPTIONS[eventName]) {
    _RNFBNativeEventEmitter.default.addListener(eventName, (...args) => {
      const event = args[0];
      if (event.appName && event.databaseId) {
        // Firestore requires both appName and databaseId to prefix
        _SharedEventEmitter.default.emit(`${event.appName}-${event.databaseId}-${eventName}`, event);
...
function getMissingModuleHelpText(namespace) {
  ...
  if (_index.isIOS || _index.isAndroid) {
...
function getAppModule() {
  ...
  const namespace = 'app';
  const nativeModule = (0, _nativeModule.getReactNativeModule)(_constants.APP_NATIVE_MODULE);
  if (!nativeModule) {
    throw new Error(getMissingModuleHelpText(namespace));
  }
