...
var _firebaseInstallations = require("@react-native-firebase/app/lib/internal/web/firebaseInstallations");
var _asyncStorage = require("@react-native-firebase/app/lib/internal/asyncStorage");
...
class AnalyticsApi {
  ...
  async _getInstallationId() {
    ...
    (0, _firebaseInstallations.makeIDBAvailable)();
    const app = (0, _firebaseInstallations.getApp)(this.appName);
    const installations = (0, _firebaseInstallations.getInstallations)(app);
    const id = await (0, _firebaseInstallations.getId)(installations);
    if (globalThis.RNFBDebug) {
      console.debug('[RNFB->Analytics][📊] Firebase Installation Id:', id);
    }
    this.installationId = id;
    (0, _firebaseInstallations.onIdChange)(installations, newId => {
  ...
  async _getCid() {
    this.cid = await (0, _asyncStorage.getItem)('analytics:cid');
    if (this.cid) {
      return this.cid;
    }
    this.cid = generateGAClientId();
    await (0, _asyncStorage.setItem)('analytics:cid', this.cid);
    if ((0, _asyncStorage.isMemoryStorage)()) {
